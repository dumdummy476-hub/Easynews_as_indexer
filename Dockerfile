...
COPY --chown=indexer:indexer requirements.txt ./
RUN pip install --no-cache-dir -r requirements.txt
COPY --chown=indexer:indexer easynews_indexer ./easynews_indexer
COPY --chown=indexer:indexer server.py easynews_client.py VERSION ./
USER indexer
EXPOSE 8081
